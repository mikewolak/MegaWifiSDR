...
/* Per-channel peak audio amplitude, tracked in resample() inner loop */
static unsigned char channel_peak[ MAX_CHANNELS ];
...
static void resample( struct channel *chan, short *buf, long offset, long count ) {
	...
	unsigned char peak = channel_peak[ chan->id ];
		...
		if( lamp || ramp ) {
			...
			if( lamp && ramp ) {
				...
				while( sidx < epos ) {
					...
					/* Track peak: |sample * max(lamp,ramp)| >> 2 */
					{ unsigned char a = ( ampl < 0 ? -ampl : ampl );
					  if( a > peak ) peak = a; }
					sidx += step;
				}
			} else {
				...
				while( sidx < epos ) {
					...
					buf_idx += 2;
					{ signed char sv = sdat[ sidx >> FP_SHIFT ];
					  unsigned char a = ( sv < 0 ? -sv : sv );
					  if( a > peak ) peak = a; }
					sidx += step;
				}
				buf_idx &= -2;
...
/*
	Returns the current pattern position in the sequence.
*/
long micromod_get_position( void ) {
	return pattern;
}

/*
	Returns the current row within the pattern.
*/
long micromod_get_row( void ) {
	return row;
}
...
*/
long micromod_get_song_length( void ) {
	return song_length;
}

/*
	Get per-channel peak audio amplitude (0-255), tracked at sample rate
	inside resample() inner loop. Peak-and-clear — resets after read.
	Scale raw 0-127 sample peaks to 0-255 for full range.
*/
long micromod_get_channel_amplitudes( unsigned char *out, long max_ch ) {
	long i, n;
	n = num_channels < max_ch ? num_channels : max_ch;
	for( i = 0; i < n; i++ ) {
		long a = channel_peak[ i ] * 2;
		out[ i ] = a > 255 ? 255 : ( unsigned char ) a;
		channel_peak[ i ] = 0;
	}
	return n;
}
