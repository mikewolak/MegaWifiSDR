 ...
 *
 * LOCAL MODIFICATIONS (perf_test/md):
 *
 * 1. Draw hook support in mw_recv_sync / mw_send_sync / mw_sock_conn_wait
 *    ────────────────────────────────────────────────────────────────────
 *    The upstream code calls TSK_superPend() with the full timeout in one
 *    shot.  During that pend the draw hook never fires, so any VDP work
 *    driven by the hook (background scroll, marquee, etc.) stalls for the
 *    entire duration of the blocking I/O operation.
 *
 *    Fix: change every TSK_superPend(tout_frames) to a 1-frame-at-a-time
 *    loop that calls the draw hook after each frame, matching the pattern
 *    already used in mw_command():
 *
 *        frames_left = tout_frames;
 *        do {
 ...
 *
 *    This keeps the display alive during recv/send/conn_wait without
 *    changing the external API or timeout semantics.
 *
 * 2. mw_sock_conn_wait timeout fix (line ~787)
 *    ──────────────────────────────────────────
 *    Upstream subtracted MW_STAT_POLL_MS (250) from a frame counter
 *    instead of MW_STAT_POLL_TOUT (~15 frames), making timeouts 17×
 *    shorter than intended.  Fixed to subtract MW_STAT_POLL_TOUT.
 *    (Also fixed and pushed in stock_ticker: commit 19d8dce)
 ****************************************************************************/
...
int16_t mw_init(uint16_t *cmd_buf, uint16_t buf_len)
{
	...
	mw_module_reset();
	// Program not active (required for the module to boot)
	uart_clr_bits(MCR, MW__PRG);

	// Verify UART scratch pad register is accessible
	UART_SPR = 0x55;
...
/* Optional per-frame draw callback registered by the application.
 * Called once per frame during mw_command() waits so the display
 * keeps updating while the ESP32 handles an HTTP round-trip. */
typedef void (*mw_draw_hook_t)(void);
static mw_draw_hook_t mw_draw_hook = NULL;

void mw_set_draw_hook(mw_draw_hook_t hook)
{
	mw_draw_hook = hook;
}
...
static enum mw_err mw_command(int16_t timeout_frames)
{
	...
	while (!done) {
		...
		// Yield 1 frame at a time so the draw hook keeps the
		// display live during long HTTP waits.
		frames_left = timeout_frames;
		do {
			tout = TSK_superPend(1);
			if (mw_draw_hook) mw_draw_hook();
			frames_left--;
		...
		if (MW_CTRL_CH == md.ch) {
			...
			frames_left = timeout_frames; // reset on data callback
		}
	}
		...
		int16_t tout_frames)
{
	struct recv_metadata md;
	bool tout;
	int16_t frames_left = tout_frames;

	lsd_recv(buf, *buf_len, &md, cmd_recv_cb);
	// Yield 1 frame at a time so the draw hook keeps the
	// display live (background scroll etc.) during waits.
	do {
		tout = TSK_superPend(1);
		if (mw_draw_hook) mw_draw_hook();
		frames_left--;
...
enum mw_err mw_detect(uint8_t *major, uint8_t *minor, char **variant)
{
	...
	// Wait a bit and take module out of reset
	TSK_superPend(MS_TO_FRAMES(30));
	mw_module_start();
	TSK_superPend(MS_TO_FRAMES(1000));
	// Re-enable Auto RTS/CTS after boot (required for ESP32-C3 comms)
...
enum mw_err mw_sock_conn_wait(uint8_t ch, int16_t tout_frames)
{
	...
	while (tout_frames > 0) {
		...
		// Sleep for MW_STAT_POLL_TOUT frames, 1 frame at a time
		// so the draw hook keeps the display live.
		{
			int16_t poll_frames = MW_STAT_POLL_TOUT;
			while (poll_frames > 0) {
				TSK_superPend(1);
				if (mw_draw_hook) mw_draw_hook();
				poll_frames--;
...
/* SDR commands */
#define MW_CMD_SDR_CONNECT    72
#define MW_CMD_SDR_TUNE       73
#define MW_CMD_SDR_DISCONNECT 74
#define MW_CMD_SDR_STATUS     75
#define MW_CMD_SDR_STATUS     75

enum mw_err mw_sdr_connect(const char *host, uint16_t port)
{
	int16_t len;
	if (!d.mw_ready) return MW_ERR_NOT_READY;
	d.cmd->cmd = MW_CMD_SDR_CONNECT;
	len = strlen(host) + 1;  /* include null */
	memcpy(d.cmd->data, host, len);
	d.cmd->data[len]     = (port >> 8) & 0xFF;
	d.cmd->data[len + 1] = port & 0xFF;
	d.cmd->data_len = len + 2;
	return mw_command(MW_AUD_TOUT * 10);  /* longer timeout for connect */
}

enum mw_err mw_sdr_tune(uint32_t freq_hz, uint8_t mode, uint16_t bandwidth)
{
	if (!d.mw_ready) return MW_ERR_NOT_READY;
	d.cmd->cmd = MW_CMD_SDR_TUNE;
	/* Pack: freq(4 big-endian) + mode(1) + bw(2 big-endian) = 7 bytes */
	d.cmd->data[0] = (freq_hz >> 24) & 0xFF;
	d.cmd->data[1] = (freq_hz >> 16) & 0xFF;
	d.cmd->data[2] = (freq_hz >> 8) & 0xFF;
	d.cmd->data[3] = freq_hz & 0xFF;
	d.cmd->data[4] = mode;
	d.cmd->data[5] = (bandwidth >> 8) & 0xFF;
	d.cmd->data[6] = bandwidth & 0xFF;
	d.cmd->data_len = 7;
	return mw_command(MW_AUD_TOUT);
}
...
{
	if (!d.mw_ready) return MW_ERR_NOT_READY;
	d.cmd->cmd = MW_CMD_SDR_DISCONNECT;
	d.cmd->data_len = 0;
	return mw_command(MW_AUD_TOUT);
}
...
enum mw_err mw_sdr_status_poll(uint32_t *w0, uint32_t *w1)
{
	enum mw_err err;
	if (!d.mw_ready) return MW_ERR_NOT_READY;
	d.cmd->cmd = MW_CMD_SDR_STATUS;
	d.cmd->data_len = 0;
	err = mw_command(MW_AUD_TOUT);
	if (err != MW_ERR_NONE) return err;
	if (w0) *w0 = d.cmd->dw_data[0];
	if (w1) *w1 = d.cmd->dw_data[1];
	return MW_ERR_NONE;
}
